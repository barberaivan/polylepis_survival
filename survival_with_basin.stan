data {
  ...
  int<lower=0> B;               // number of basins (random effect)
...
parameters {
  ...
  vector[B] e_basin_raw;             // basin random effects
  real<lower=0> sigma_plot_raw;      // raneff sd
  real<lower=0> sigma_basin_raw;
}

transformed parameters {
  vector[K] b;                   // fixed parameters
  vector[P] e_plot;              // plot random effects
  vector[B] e_basin;             // basin random effects
  ...
  sigma_basin = sigma_basin_raw * prior_sigma_sd;
  e_plot = e_plot_raw * sigma_plot;
  e_basin = e_basin_raw * sigma_plot;
...
model {
  ...
  sigma_basin_raw ~ std_normal();
  e_plot_raw ~ std_normal();
  e_basin_raw ~ std_normal();

  // likelihood for identified trees
  y ~ bernoulli_logit(X * b + Zp * e_plot + Zb * e_basin);
